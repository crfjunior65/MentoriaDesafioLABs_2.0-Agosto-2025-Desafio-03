...
create_task_definition() {
    ...
    if [ $? -ne 0 ]; then
        log_error "Erro ao processar task definition com jq" >&2
        rm -f "$temp_file"
        return 1
    fi
    ...
    local new_revision=$(aws ecs register-task-definition --region $region --cli-input-json file://"$new_temp_file" --query 'taskDefinition.revision' --output text 2>/dev/null)
...
deploy() {
    ...
    if [ $create_exit_code -ne 0 ] || [ -z "$new_revision" ]; then
        log_error "Falha ao obter revision da nova task definition"
        log_error "Exit code: $create_exit_code"
        log_error "Revision: '$new_revision'"
        exit 1
    fi
